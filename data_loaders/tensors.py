...
# Conditional collate for 2 persons
def ccollate(batch):
    notnone_batches = [b for b in batch if b is not None]
    nfeats = notnone_batches[0]['inp'].shape[1] # [25, 12, 60]
    databatch = [b['inp'][:,nfeats//2:] for b in notnone_batches]
    cdatabatch = [b['inp'][:,:nfeats//2] for b in notnone_batches]
    if 'lengths' in notnone_batches[0]:
        lenbatch = [b['lengths'] for b in notnone_batches]
    else:
        lenbatch = [len(b['inp'][0][0]) for b in notnone_batches]
    ...
    databatchTensor = collate_tensors(databatch)
    cdatabatchTensor = collate_tensors(cdatabatch)
    lenbatchTensor = torch.as_tensor(lenbatch)
    maskbatchTensor = lengths_to_mask(lenbatchTensor, databatchTensor.shape[-1]).unsqueeze(1).unsqueeze(1) # unqueeze for broadcasting
