    ...
    def _load(self, ind, frame_ix):
        ...
        if pose_rep == "xyz" or self.translation:
            if getattr(self, "_load_joints3D", None) is not None:
                ...
                joints3D = self._load_joints3D(ind, frame_ix)
                if shuffle_or_not:
                    tmp = np.zeros_like(joints3D)
                    tmp[:,:,0:3] = joints3D[:,:,3:6]
                    tmp[:,:,3:6] = joints3D[:,:,0:3]
                    joints3D = tmp
                if self.num_person > 1:
                    base_trans = copy.deepcopy(joints3D[0, 0, 0:3])
                    joints3D_x = joints3D
