...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, num_frames=1, sampling="conseq", sampling_step=1, split="train",
        ...
        self.num_person = kwargs.get('num_person', 1)
    ...
    def _load(self, ind, frame_ix):
        ...
        if pose_rep == "xyz":
            if getattr(self, "_load_joints3D", None) is not None:
                ...
                ret = np.concatenate([joints3D, pose], axis=1) # [T, 25, 3]
                ret = to_torch(ret)
            ret = ret.permute(1, 2, 0).contiguous() # [V, C, T]
            return ret.float()
        if pose_rep == "xyz" or self.translation:
            if getattr(self, "_load_joints3D", None) is not None:
                ...
                if self.num_person > 1:
                    base_trans = copy.deepcopy(joints3D[0, 0, 0:3]) # BUG fix joints3D[T, 1, 6]
                    joints3D_x = joints3D
                    for ii in range(self.num_person):
                        # joints3D[:,:,3*ii:3*(ii+1)] = joints3D[:,:,3*ii:3*(ii+1)] - joints3D[0, 0, 3*ii:3*(ii+1)]
                        # BUG fix, for each person, minus the root translation of the first frame of the first person, 2023.03.22 Derek
                        joints3D[:,:,3*ii:3*(ii+1)] = joints3D[:,:,3*ii:3*(ii+1)] - base_trans
                else:
                    joints3D = joints3D - joints3D[0, 0, :]
