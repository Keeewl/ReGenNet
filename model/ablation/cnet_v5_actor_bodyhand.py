...
class CNetV5ActorBodyHand(CNetV5):
    """Ablation of CNetV5 with actor tokens encoded as body/hand only."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        del self.actor_global_input_process
        del self.actor_part_input_processes

        self.actor_token_names = ["body", "hand"]
        self.actor_num_tokens_per_frame = 2
    ...
    def _build_actor_tokens(self, actor):
        actor_body = actor[:, self.body_joint_ids]
        actor_hand = actor[:, self.hand_joint_ids]
        body_tokens = self.actor_body_input_process(actor_body)
        hand_tokens = self.actor_hand_input_process(actor_hand)
        actor_tokens = torch.stack([body_tokens, hand_tokens], dim=2)
        actor_tokens = actor_tokens.reshape(
            actor_tokens.shape[0], -1, actor_tokens.shape[-1]
        )
        num_frames = body_tokens.shape[1]
        token_type_ids = torch.arange(
            self.actor_num_tokens_per_frame,
