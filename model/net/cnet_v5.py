...
class ActorEncoder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.token_type_emb = nn.Embedding(num_token_types, dim)
...
class CNetV5TransformerLayer(nn.Module):
    """
...
class CNetV5(nn.Module):
    """
    two-stream conditional denoiser
    = body stream + hand stream + multi-part actor tokens + PIA + ParCo coordination
    """
    def __init__(
        ...
        actor_num_layers=2, actor_dim=256, dropout=0.1, ablation=None, activation="gelu",
        legacy=False, data_rep="rot6d", dataset="chi3d", clip_dim=512, arch="offline",
        cm_mode="concat", body_model="smplx", wo_pos_emb=False, clip_version=None, **kargs
    ):
        super().__init__()
        ...
        if self.arch not in ["offline", "online"]:
            raise ValueError("CNet v5 only supports arch='offline' or 'online'.")
        if self.dataset not in ["chi3d", "interx"]:
            raise ValueError("CNet v5 only supports dataset='chi3d' or 'interx'.")

        self.part_names = PART_NAMES
        for name in self.part_names:
            part_ids = torch.tensor(PART_JOINT_IDS[name], dtype=torch.long)
            self.register_buffer(f"part_joint_ids_{name}", part_ids)

        self.actor_global_input_process = InputProcess(
            self.data_rep, self.input_feats, self.actor_dim
        )
        self.actor_part_input_processes = nn.ModuleDict()
        for name in self.part_names:
            part_ids = getattr(self, f"part_joint_ids_{name}")
            self.actor_part_input_processes[name] = InputProcess(
                self.data_rep, int(part_ids.numel()) * self.nfeats, self.actor_dim
            )
        ...
        self.actor_encoder = ActorEncoder(
            ...
            num_token_types=1 + len(self.part_names),
            mlp_ratio=self.ff_size / self.actor_dim,
            dropout=self.dropout,
            activation=self.activation,
        )
        self.actor_to_pia = nn.Linear(self.actor_dim, self.latent_dim)
        ...
        if body_model == "smpl":
            ...
            raise ValueError("CNet v5 only supports body_model='smpl' or 'smplx'.")
    ...
    def _build_actor_tokens(self, actor):
        ...
        for name in self.part_names:
            part_ids = getattr(self, f"part_joint_ids_{name}")
            part = actor[:, part_ids]
            part_tokens.append(self.actor_part_input_processes[name](part))
        tokens = [actor_global] + part_tokens
        actor_tokens = torch.stack(tokens, dim=2)
        actor_tokens = actor_tokens.reshape(actor_tokens.shape[0], -1, actor_tokens.shape[-1])
        num_frames = actor_global.shape[1]
        num_tokens_per_frame = 1 + len(self.part_names)
        token_type_ids = torch.arange(
            num_tokens_per_frame, device=actor_tokens.device, dtype=torch.long
        ).repeat(num_frames)
    ...
    def forward(self, x, timesteps, y=None, return_attn=False):
        ...
        if online:
            ...
            actor_attn_mask = _build_actor_causal_mask(
                nframes, num_tokens_per_frame, actor_tokens.device
            )
