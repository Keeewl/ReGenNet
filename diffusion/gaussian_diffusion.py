...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.lambda_transl = lambda_transl
    ...
    def p_mean_variance(
        ...
        y_kwargs = model_kwargs.get('y', None)
        if isinstance(y_kwargs, dict) and 'inpainting_mask' in y_kwargs and 'inpainted_motion' in y_kwargs:
            inpainting_mask, inpainted_motion = y_kwargs['inpainting_mask'], y_kwargs['inpainted_motion']
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        get_xyz = lambda sample: enc.rot2xyz(sample, mask=None, pose_rep=enc.pose_rep, translation=enc.translation,
                                             ...
                                             jointstype=self.body_model,  # 3.4 iter/sec
                                             vertstrans=False, num_person=self.num_person)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.lambda_fc > 0.:
                with torch.autograd.set_detect_anomaly(True):
                    if self.data_rep == 'rot6d' and dataset.dataname in ['chi3d', 'interx']:
                        ...
                        gt_joint_vel = torch.linalg.norm(gt_joint_xyz[:, :, :, 1:] - gt_joint_xyz[:, :, :, :-1], axis=2)  # [BatchSize, 4, Frames]
                        fc_mask = torch.unsqueeze((gt_joint_vel <= 0.01), dim=2).repeat(1, 1, 3*self.num_person, 1)
                        pred_joint_xyz = model_output[:, relevant_joints, :, :]  # [BatchSize, 4, 3, Frames]
                        pred_vel = pred_joint_xyz[:, :, :, 1:] - pred_joint_xyz[:, :, :, :-1]
                        pred_vel[~fc_mask] = 0
                        terms["fc"] = self.masked_l2(pred_vel,
                                                    torch.zeros(pred_vel.shape, device=pred_vel.device),
            ...
            if self.lambda_orient > 0. or self.lambda_body > 0. or self.lambda_transl > 0.:
                cmotion = model_kwargs["y"]["cmotion"]
                cmotion_xyz = get_xyz(cmotion)
            if self.lambda_orient > 0.:
                # 1. Relative orientation loss
                cmotion_orient = cmotion[:,0:1,:,:]
                target_orient, model_output_orient = target[:,0:1,:,:], model_output[:,0:1,:,:]
                cmotion_orient_rm = geometry.rotation_6d_to_matrix(cmotion_orient.permute(0, 1, 3, 2))
                target_orient_rm = geometry.rotation_6d_to_matrix(target_orient.permute(0, 1, 3, 2))
                model_output_orient_rm = geometry.rotation_6d_to_matrix(model_output_orient.permute(0, 1, 3, 2))
                gt_orient_diff = torch.linalg.norm(geometry.matrix_to_axis_angle(torch.matmul(cmotion_orient_rm.permute(0, 1, 2, 4, 3), target_orient_rm)), axis=-1)
                model_output_orient_diff = torch.linalg.norm(geometry.matrix_to_axis_angle(torch.matmul(cmotion_orient_rm.permute(0, 1, 2, 4, 3), model_output_orient_rm)), axis=-1)
                orient_diff = self.masked_l2(gt_orient_diff, model_output_orient_diff, mask.squeeze(1))
                terms["orient"] = orient_diff
            if self.lambda_body > 0.:
                # 2. Relative body pose loss
                gt_joint_diff = torch.linalg.norm(cmotion_xyz - target_xyz, axis=2)
                model_output_joint_diff = torch.linalg.norm(cmotion_xyz - model_output_xyz, axis=2)
                body_diff = self.masked_l2(gt_joint_diff, model_output_joint_diff, mask.squeeze(1))
            ...
            if self.lambda_transl > 0.:
                ...
                terms["transl"] = transl_diff

            terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            (self.lambda_vel * terms.get('vel_mse', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_mse', 0.)) + \
                            (self.lambda_fc * terms.get('fc', 0.)) + \
                            (self.lambda_orient * terms.get('orient', 0.)) + \
                            (self.lambda_body * terms.get('body', 0.)) + \
                            (self.lambda_transl * terms.get('transl', 0.))
        else:
