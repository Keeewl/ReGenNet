...
\title{HiReact: Decoupling Global Motion Synthesis and Local Contact Refinement for Contact-Realistic Human Reaction Generation}
...
Human reaction generation aims to synthesize realistic reactor motions conditioned on an observed actor motion sequence. Existing methods have achieved promising global motion realism, but often struggle to model fine-grained interaction details, particularly around sparse and temporally localized physical contacts. In many interaction scenarios, globally plausible reactions can still exhibit unrealistic local behaviors such as inaccurate hand alignment, unstable contact timing, or implausible contact geometry. These failures arise because global motion synthesis and local contact modeling operate at different spatial and optimization granularities.
To address this issue, we propose HiReact, a hierarchical framework that decouples human reaction generation into global motion synthesis and local contact refinement. In Stage I, HiReact performs actor-conditioned reaction synthesis using role-specific multi-scale interaction representations. The actor motion is decomposed into global and part-level interaction cues, while the reactor motion is generated through coordinated body and hand streams to preserve both full-body coherence and fine-grained local responsiveness. In Stage II, HiReact identifies contact-relevant temporal regions and performs localized residual refinement to improve interaction fidelity without regenerating the entire motion sequence.
Experiments on Inter-X and CHI3D demonstrate that HiReact improves both global reaction quality and local contact accuracy over prior methods. In particular, HiReact achieves more realistic hand-centric interactions and better contact geometry while maintaining temporally coherent full-body motion. Our results suggest that decoupling global reaction synthesis from local contact refinement is effective for high-fidelity human reaction generation.
...
Human reaction generation aims to synthesize the motion of a reactor conditioned on the observed motion of an interacting actor. As an asymmetric form of human-human interaction modeling, it is important for interactive animation, virtual humans, behavior simulation, and human-centered embodied agents. 
Recent diffusion-based approaches have significantly improved the realism and temporal coherence of generated interactions.
However, realistic human reactions require more than globally plausible motion trajectories. 
In many interaction scenarios, local physical behaviors, especially hand-centric contacts, play a critical role in perceived realism. 
As illustrated in Fig.~\ref{fig:intro}, existing methods often generate reactions that appear reasonable at the global level but exhibit noticeable local artifacts, such as inaccurate hand alignment, unstable contact timing, or physically implausible contact geometry.

Most existing reaction generation methods model the reactor as a holistic motion sequence and optimize interaction quality globally throughout the generation process
While such designs are effective for learning coarse interaction dynamics, sparse local contacts remain difficult to optimize jointly with full-sequence motion realism. Contact-related errors are often temporally localized and geometrically sensitive, causing them to receive limited attention during global sequence generation. 
Prior works have incorporated contact-aware constraints, interaction losses, or contact-guided sampling strategies to improve interaction fidelity~\cite{xu2024regennet, zhang2025reactffusion}. 
Contact-aware refinement has also been explored in hand-object and human-object interaction tasks, where localized interaction geometry is corrected after coarse estimation~\cite{zhou2022toch, diller2024cg, cha2024text2hoi}. 
However, these approaches still primarily optimize interaction quality within a unified full-sequence generation process, making it difficult to selectively improve sparse local contact regions without disturbing globally coherent motion.

We argue that human reaction generation should be treated as a hierarchical interaction modeling problem with decoupled global and local objectives. 
At the interaction level, actor motions provide cues at multiple spatial scales: global body motion defines the overall interaction context, while localized body-part movements indicate where and how interactions occur. 
At the optimization level, globally coherent reaction synthesis and locally accurate contact modeling require different modeling granularities. 
This motivates decoupling human reaction generation into two complementary stages: global motion synthesis and local contact refinement.

Based on this observation, we propose \textbf{HiReact}, a \textbf{Hi}erarchical framework that decouples human \textbf{React}ion generation into global synthesis and localized contact refinement. In Stage I, HiReact performs actor-conditioned reaction synthesis using role-specific multi-scale interaction modeling. The actor motion is decomposed into global and part-level interaction cues, while the reactor motion is generated through coordinated body and hand streams to preserve both full-body coherence and fine-grained local responsiveness. In Stage II, HiReact identifies contact-relevant temporal windows from the initial reaction and performs residual local refinement to improve interaction fidelity without regenerating the entire motion sequence. This design allows HiReact to first generate globally coherent reactions and then selectively refine temporally sparse but interaction-critical local contact regions.

Experiments on Inter-X~\cite{xu2024regennet} and CHI3D~\cite{fieraru2020three} demonstrate that HiReact consistently improves both global motion quality and contact-oriented interaction metrics over prior methods. In particular, HiReact produces more realistic hand-centric interactions and better local contact geometry while preserving temporally coherent full-body motion. Extensive ablation studies further validate the effectiveness of decoupling global motion synthesis from local contact refinement, as well as the proposed multi-scale actor-reactor interaction modeling strategy.

Our contributions are summarized as follows. 
\begin{itemize}
    \item We propose HiReact, a hierarchical framework that decouples human reaction generation into global motion synthesis and local contact refinement, addressing the optimization mismatch between full-body motion realism and localized interaction fidelity.
    \item Experiments on Inter-X and CHI3D demonstrate improved global motion realism and local contact fidelity over prior reaction generation methods.
\end{itemize}
